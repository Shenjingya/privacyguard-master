...
public class MyVpnService extends VpnService implements Runnable {
    ...
    //Network
    private MyNetworkHostNameResolver hostNameResolver;//分解器
    ...
    void buildNotification(int notifyId, int frequency, LeakReport leak) {
        ...
        int idx = leak.metaData.destHostName.lastIndexOf('.');//指定字符串出现的位置
        String destNetwork;
        if (idx > 0)
        ...
        // started Activity.
        //堆栈构建器对象将为已经启动的Activity包含一个人工后台堆栈
    ...
    private void stop() {
        ...
        try {
            ...
            Logger.e(TAG, e.toString() + "\n" + Arrays.toString(e.getStackTrace()));//tostring转字符串
        }
        mInterface = null;
    }
